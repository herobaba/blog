...
<div class="maincont">
    <div class="head-top">
        ...
        <dl>
            ...
            <dd>
                <h1 class="username"><b> 欢迎 <i style="background-color:red">{{$name}}</i> </b></h1>
                <ul>
    ...
    <ul class="pronav">
        @foreach($cart as $k=>$v)
        <li><a href="{{url('/index/prolistt',['cate_id'=>$v->cate_id])}}" >{{$v->cate_name}}</a></li>
        @endforeach
        <div class="clearfix"></div>
    </ul><!--pronav/-->
    <div class="index-pro1">
        ...
        <div class="index-pro1-list">
            <dl>
                <dt><a href="proinfo?goods_id={{$v->goods_id}}"><img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" /></a></dt>
                <dd class="ip-text"><a href="proinfo?goods_id={{$v->goods_id}}">{{$v->goods_name}}</a><span>已售：{{$v->goods_score}}</span></dd>
