...
<body>
    <div class="maincont">
     ...
     <div id="sliderA" class="slider">
      @foreach($data as $k=>$v)
      <img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" />
      <img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" />
      <img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" />
      <img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" />
      <img src="{{config('app.img_url')}}{{$v->goods_mid_pic}}" />
     ...
     <form action="pl" method="post">
      {{csrf_field()}}
      <input type="radio" name="p_dengji" checked class="wu"value="★">一级
       <input type="radio" name="p_dengji" class="wu"value="★★">2级
       <input type="radio" name="p_dengji" class="wu"value="★★★">3级
       <input type="radio" name="p_dengji" class="wu"value="★★★★">4级
       <input type="radio" name="p_dengji" class="wu"value="★★★★★">5级
       <br>
    <br>
      <input type="hidden" name="goods_id" id="goods_id" value="{{$v->goods_id}}">
      邮箱:<input type="text" name="p_email" id="p_email"><br>
      <br>
       <textarea name="p_content" id="ping" cols="30" rows="4"></textarea><br>     
  ...
  @endforeach
     ...
     <div class="proinfoList">
      <img src="{{config('app.img_url')}}{{$v->goods_big_pic}}" width="700" height="500" />
     </div><!--proinfoList/-->
     <div class="proinfoList">
      {{$v->goods_details}}
     </div><!--proinfoList/-->
...
$(document).on('click','.pagination a',function(){
 ...
 $(function () {
    ...
    $("#tijiao").click(function(){
      //alert(1);
      //var wu=$(":radio").prop('checked').val();
      data={};
      data.p_dengji=$(".wu:checked").val();
      data.p_content=$("#ping").val();
      data.goods_id=$("#goods_id").val();
      data.p_email=$("#p_email").val();
      //alert(data);
      $.post(
        'pl',
        data,
